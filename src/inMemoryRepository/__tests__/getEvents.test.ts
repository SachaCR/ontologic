...
describe('InMemoryRepository.getEvents', () => {
  ...
  describe('pagination', () => {
    ...
    it('respects limit', async () => {
      const events = (await repo.getEvents('1', { limit: 2, offset: 0 }))._unsafeUnwrap();
      expect(events).toEqual([event1, event2]);
    });

    it('respects offset', async () => {
      const events = (await repo.getEvents('1', { limit: 10, offset: 1 }))._unsafeUnwrap();
      expect(events).toEqual([event2, event3]);
    });

    it('respects both limit and offset', async () => {
      const events = (await repo.getEvents('1', { limit: 1, offset: 1 }))._unsafeUnwrap();
      expect(events).toEqual([event2]);
    });

    it('returns empty array when offset exceeds event count', async () => {
      const events = (await repo.getEvents('1', { limit: 10, offset: 99 }))._unsafeUnwrap();
