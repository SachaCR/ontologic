...
export class AddItemToOrderUseCase implements UseCase<
...
> {
  ...
  async execute(
    ...
    const addItemEvent = result.value;

    const saveResult = await this.orders.saveWithEvents(order, addItemEvent);
