...
>({
  ...
  // because there is none yet — which is why it is declared separately.
  creation: {
    event: "CART_CREATED",
    applier: ({ event }) => ({
  ...
  // One applier per remaining event. The map is exhaustive over `CartEvent`:
  // leave one out and this does not compile.
  appliers: {
...
| **Creation event**        | Declared separately, takes no prior state, not replayable onto a snapshot   |
