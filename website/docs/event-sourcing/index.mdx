...
Hover or tab to an event to see its payload. Click one to rewind the cart to that version.
...
// The event that brings a cart into existence. It receives no prior state,
// because there is none yet — which is why it is mounted separately.
...
const { state, version } = cartProjection.apply({
  events: [cartCreated, mugAdded, teeAdded],
});
...
Note that the creation event is not replayable on top of a snapshot. A cart that already exists cannot be created again, so passing a stream that still starts with `CART_CREATED` alongside a snapshot is an error rather than a silent re-initialization.
...
| **Creation event**        | Mounted separately, takes no prior state, not replayable onto a snapshot    |
